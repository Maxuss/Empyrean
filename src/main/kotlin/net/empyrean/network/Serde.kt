...
fun <T> encodePacket(serializer: SerializationStrategy<T>, packet: T, out: FriendlyByteBuf) =
    NetworkEncoder(out).encodeSerializableValue(serializer, packet)

inline fun <reified T> decodePacket(from: FriendlyByteBuf): T = decodePacket(serializer(), from)

fun <T> decodePacket(serializer: DeserializationStrategy<T>, from: FriendlyByteBuf) =
    NetworkDecoder(from).decodeSerializableValue(serializer)